...
<!-- catalog solution -->
<xs:import namespace="http://schemas.nav.gov.hu/NTCA/2.0/common"/> + catalog file
----
...
Due to the nature of the project, it is important that different projects remain compatible with each other.
Therefore, it may happen that an imported schema uses an older version in one project, while another schema uses a newer version.
To avoid conflicts, the project's groupId contains the major version number, so the versions remain separated.

.Multiple versions used in parallel
[source, xml]
----
